...
- **Two triggers**: weekly cron (Mon 08:00 Australia/Sydney) AND webhook `cliniciqtrafficking` (POST, `?key=<secret>` checked by `secret ok?`) fired by a Netlify **deploy_succeeded** outgoing hook (hook id `6a9897a6cb740534464351dc` on site `ed7a559e-b9b5-4d50-ae0d-956ecd36480d`) — so every push re-pings the engines automatically. Real secret lives only in the live workflow + Netlify hook (public repo — artifact keeps `TRAFFIC_PING_SECRET_PLACEHOLDER`).
